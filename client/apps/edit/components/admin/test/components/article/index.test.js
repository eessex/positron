...
describe('AdminFeaturing', () => {
    ...
    response = {
      body: {
        data: {
          articles: [{
            id: '123',
            title: 'How a Dissatisfied Housewife Was Saved by Radical Performance'
          }]
        }
      }
    }
  ...
  describe('Rendering', () => {
    ...
    it('Renders tier buttons', () => {
      ...
      expect(component.find('button').at(0).text()).toBe('Tier 1')
    ...
    it('Renders layout buttons if editorial', () => {
      ...
      expect(component.find('button').at(4).text()).toBe('Standard')
    ...
    it('Renders related articles autocomplete', () => {
      ...
      expect(component.find(AutocompleteList).at(1).getElement().props.url).toMatch('articles')
    })
  })
  ...
  describe('Editing', () => {
    ...
    it('Can change indexable checkbox', () => {
      ...
      const component = getWrapper(props)
      component.find('.flat-checkbox').at(0).simulate('click')
    ...
    it('Can change Google news checkbox', () => {
      ...
      const component = getWrapper(props)
      component.find('.flat-checkbox').at(1).simulate('click')

      expect(props.onChangeArticleAction.mock.calls[0][0]).toBe('exclude_google_news')
      expect(props.onChangeArticleAction.mock.calls[0][1]).toBe(false)
    })
  })
})
